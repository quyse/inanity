...
public:
	...
	virtual void DrawVisualElement(VisualElement element, float x1, float y1, float x2, float y2) = 0;
	/// Получить размеры текста.
	virtual Size GetTextSize(VisualFont font, const String& text) const = 0;
	/// Напечатать текст.
	virtual void DrawRectText(VisualFont font, const String& text, float x1, float y1, float x2, float y2) = 0;
};
