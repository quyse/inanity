...
{
private:
	ptr<UdpSocket> socket;
	CriticalSection cs;

	void Connected(const Service::UdpSocketHandler::Result& result)
	{
		try
		{
			socket = result.GetData();

			socket->SetReceiveHandler(UdpSocket::ReceiveHandler::Bind<ChatClient>(this, &ChatClient::Received));
		}
	...
	void Received(const UdpSocket::ReceiveHandler::Result& result)
	{
		CriticalCode cc(cs);
		try
		{
...
public:
	...
	{
		service->ConnectUdp(host, 8080, Service::UdpSocketHandler::Bind<ChatClient>(this, &ChatClient::Connected));
	}
...
private:
	...
	public:
		Client(ptr<ChatServer> server, ptr<UdpSocket> socket)
		: server(server), socket(socket)
		{
			socket->SetReceiveHandler(UdpSocket::ReceiveHandler::Bind<Client>(this, &Client::Received));
		}
	...
	void Accepted(const Service::UdpPacketHandler::Result& result)
	{
		CriticalCode cc(cs);
		try
		{
			ptr<UdpPacket> packet = result.GetData();
			ptr<Client> client = NEW(Client(this, packet->CreateSocket()));
...
public:
	...
	{
		listener = service->ListenUdp(8080, Service::UdpPacketHandler::Bind<ChatServer>(this, &ChatServer::Accepted));
	}
